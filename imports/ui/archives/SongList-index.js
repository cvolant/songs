  ...
  useEffect(() => {
    if (search && (
      ...
      const subscription = Meteor.subscribe('songs', { search, filters }, function () {
        const subscribedSongs = Songs.find({}).fetch();
        setSongs(subscribedSongs);
        console.log('From SongList, useEffect[JSON.stringify(search)], subscription callback. subscribedSongs.length:', subscribedSongs.length);
        stopLoading();
      });
      return () => {
...
export default withTracker(() => {
  ...
  const favoriteSongs = Meteor.user()
    && Meteor.user().userSongs
    && Meteor.user().userSongs.favoriteSongs;
